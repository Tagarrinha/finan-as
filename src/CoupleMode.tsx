...
export default function CoupleMode({ userId, userEmail, userName, expCats, accent, accentDark, cardBg, cardBorder, subtext, positive, negative }: Props) {
  ...
  // ── KEY FIX: search by user1_id, user2_id OR user2_email (pending invites) ──
  async function loadCouple() {
    ...
    if(c) {
      // If found via email but user2_id not set yet, update it automatically
  ...
  async function sendInvite() {
    ...
    if(error){setInviteErr("Erro ao enviar convite. Tenta novamente.");setInviting(false);return;}
  ...
  async function acceptInvite() {
    ...
    if(!error&&data){
      setCouple(data as Couple);
      const {data:acc} = await supabase.from("couple_account")
        .insert({couple_id:couple.id, saldo:0, contribuicao_user1:0, contribuicao_user2:0})
        .select().single();
  ...
  async function addExpense() {
    ...
    const {data,error} = await supabase.from("couple_expenses").insert({
      ...
      data:form.data, split_user1:s1, split_user2:s2, pago_por:userId, liquidado:false,
    }).select().single();
  ...
  async function addSettlement() {
    ...
    const {data,error} = await supabase.from("couple_settlements")
      .insert({couple_id:couple.id, pago_por:userId, valor:Number(settleValor), nota:settleNota})
      .select().single();
  ...
  if(!couple) return (
    <div style={{fontFamily:"'Sora',sans-serif"}}>
      ...
      <div style={{background:"rgba(255,255,255,0.03)",border:`1px solid ${cardBorder}`,borderRadius:12,padding:"14px 16px",marginTop:12}}>
        ...
        {["O teu parceiro/a abre a app e vai ao tab 👫 Casal","O convite aparece automaticamente","Aceita e ficam ligados em modo casal","Adicionam despesas conjuntas — 50/50 ou personalizado","Veem quem deve quanto a quem em tempo real"].map((t,i)=>(
      ...
      <div style={{fontFamily:"'Sora',sans-serif"}}>
        {isSender?(
          <div style={{background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.3)",borderRadius:16,padding:"24px",textAlign:"center" as const}}>
            ...
            <div style={{background:"rgba(245,158,11,0.12)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:10,padding:"10px 14px",fontSize:12,color:"#f59e0b",marginBottom:16}}>
              O convite aparece automaticamente quando o/a parceiro/a abre o tab Casal.
            </div>
    ...
    <div style={{fontFamily:"'Sora',sans-serif"}}>
      ...
      {tab==="conta"&&<>
        ...
        <div style={{background:cardBg,border:`1px solid ${cardBorder}`,borderRadius:12,padding:"14px 16px"}}>
          <div style={{fontSize:10,fontWeight:700,color:subtext,textTransform:"uppercase" as const,letterSpacing:"0.08em",marginBottom:12}}>Despesas conjuntas recentes</div>
          ...
          :expenses.slice(0,5).map(e=>{
            ...
            return(
              <div key={e.id} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 0",borderBottom:`1px solid ${cardBorder}`}}>
                ...
                <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:"#e2e8f0"}}>{e.descricao}</div><div style={{fontSize:10,color:subtext}}>A tua parte: <span style={{color:MY_COLOR,fontWeight:700}}>{fmt(myShare)}</span></div></div>
          ...
          return(
            <div key={e.id} style={{background:cardBg,border:`1px solid ${cardBorder}`,borderRadius:12,padding:"12px 14px",marginBottom:10}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                <div style={{background:`${MY_COLOR}12`,borderRadius:8,padding:"6px 10px",display:"flex",justifyContent:"space-between"}}>
                  <span style={{fontSize:11,color:MY_COLOR,fontWeight:600}}>{userName}</span>
                  <span style={{fontSize:12,fontWeight:700,color:MY_COLOR}}>{fmt(myShare)}</span>
                </div>
                <div style={{background:`${PARTNER_COLOR}12`,borderRadius:8,padding:"6px 10px",display:"flex",justifyContent:"space-between"}}>
      ...
      {tab==="acerto"&&<>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
          {[{nome:userName,cor:MY_COLOR,paid:myPaid,due:myExpTotal},{nome:partnerName,cor:PARTNER_COLOR,paid:partnerPaid,due:partnerExpTotal}].map((p,i)=>(
