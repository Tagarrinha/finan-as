...
const BUDGET_TARGETS = { necessidade: 75, desejo: 10, investimento: 15 };
        ...
        return (
          <button
            ...
            style={{
              padding: '10px 6px',
              border: `1.5px solid ${
                active ? meta.color : 'rgba(255,255,255,0.09)'
              }`,
              borderRadius: 10,
...
export default function Financas() {
  ...
  const [monthlyRev, setMonthlyRev] = useLocalStorage('fin_monthlyRev', {
    pessoal: {},
    clinica: {},
  });
  ...
  const expCats =
    world === 'pessoal' ? PERSONAL_EXPENSE_CATS : CLINIC_EXPENSE_CATS;
  const incCats =
    world === 'pessoal' ? PERSONAL_INCOME_CATS : CLINIC_INCOME_CATS;
  ...
  const myExpenses = useMemo(
    () =>
      expenses.filter((e) => {
        ...
        if (fYear !== 'todos' && String(d.getFullYear()) !== fYear)
          return false;
  ...
  const totalExp = myExpenses.reduce((s, e) => s + e.valor, 0);
  const totalInc = myIncomes.reduce((s, i) => s + i.valor, 0);
  const balance = totalInc - totalExp;

  const byType = useMemo(() => {
    const map = { necessidade: 0, desejo: 0, investimento: 0 };
    myExpenses.forEach((e) => {
      if (map[e.tipo] !== undefined) map[e.tipo] += e.valor;
    });
    return map;
  }, [myExpenses]);
  ...
  const revArr = monthlyRev[world]?.[revYear] || new Array(12).fill(0);
      ...
      expenses
        .filter(
          (e) =>
            ...
            String(new Date(e.data).getFullYear()) === revYear
        )
        .reduce((s, e) => s + e.valor, 0)
    ),
    1
  ...
  const overBudget = Object.entries(BUDGET_TARGETS).filter(([type, target]) => {
  ...
  function addExpense() {
    ...
    setExpenses((p) => [
      ...p,
      { ...expForm, id: Date.now(), valor: Number(expForm.valor), world },
    ]);
  ...
  function addIncome() {
    ...
    setIncomes((p) => [
      ...p,
      { ...incForm, id: Date.now(), valor: Number(incForm.valor), world },
    ]);
  ...
  const S = {
    ...
    header: {
      background:
        world === 'clinica'
          ? 'linear-gradient(135deg,#0c1a2e,#0c2236)'
          : 'linear-gradient(135deg,#1a0c08,#2d150a)',
      padding: '24px 20px 0',
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 12, color: '#475569', marginTop: 2 }}>
              Pessoal & Clínica Privada ·{' '}
              <span style={{ color: '#34d399' }}>dados guardados ✓</span>
            </div>
          </div>
          <button
            onClick={() => {
      ...
      <div style={S.body}>
          ...
          <>
            <div style={S.sg}>
              <StatCard
                ...
                value={totalInc}
                color="#34d399"
                sub={`${myIncomes.length} entradas`}
              />
              <StatCard
                label="Despesas"
                value={totalExp}
                color="#fb7185"
                sub={`${myExpenses.length} items`}
              />
            </div>
            <div
              style={{
            ...
            <div style={S.card}>
              <SectionTitle>
                Distribuição por tipo — metas orçamentais
              </SectionTitle>
              {Object.entries(TYPE_META).map(([type, meta]) => {
                const actual = byType[type] || 0;
                    ...
                    >
                      <div
                        style={{
                          ...
                          background: over ? '#ef4444' : meta.color,
                          borderRadius: 99,
                          transition: 'width .5s ease',
                          overflow: 'hidden',
                        }}
                      ...
                      >
                        ...
                        <div>
                          ...
                          <span style={{ fontSize: 11, color: '#f87171' }}>
                            +{fmt(actual - targetAmt)} acima da meta (
                            {actualPct - target}% extra)
                          </span>
                        </div>
                      </div>
                    )}
                ...
                .map((c) => (
                  <div key={c.id} style={{ marginBottom: 10 }}>
                    ...
                    <ProgressBar
                      value={c.total}
                      max={maxCat}
                      color={TYPE_META[c.type]?.color || '#6b7280'}
                      height={4}
                    />
                  </div>
                ))}
            ...
            <div style={S.card}>
              ...
              {byIncCat.filter((c) => c.total > 0).length === 0 && (
                <div
                  style={{
                    color: '#374151',
                    fontSize: 13,
          ...
          <>
            ...
            {overBudget.length > 0 && (
              ...
              >
                ...
                >
                  ⚠️ Orçamento excedido neste período
                </div>
                {overBudget.map(([type]) => {
                  const actual = byType[type] || 0;
                  const target = BUDGET_TARGETS[type];
                  ...
                  return (
                    ...
                    >
                      {meta.icon} {meta.label}: {pct(actual, totalExp)}% usado
                      (meta {target}%) · excesso de {fmt(actual - targetAmt)}
                    </div>
                  );
                })}
              </div>
            ...
            <div style={S.card}>
              ...
              <div style={S.row2}>
                <div>
                  ...
                  <input
                    ...
                    value={expForm.desc}
                    onChange={(e) =>
                      setExpForm((f) => ({ ...f, desc: e.target.value }))
                    }
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    value={expForm.valor}
                    onChange={(e) =>
                      setExpForm((f) => ({ ...f, valor: e.target.value }))
                    }
                  />
                </div>
              </div>
              <div style={S.row2}>
                ...
                <div>
                  ...
                  <input
                    ...
                    value={expForm.data}
                    onChange={(e) =>
                      setExpForm((f) => ({ ...f, data: e.target.value }))
                    }
                  />
                </div>
              </div>
                      ...
                      return (
                        <button
                          ...
                          style={{
                            padding: '7px 13px',
                            border: `1.5px solid ${
                              active ? accent : 'rgba(255,255,255,0.13)'
                            }`,
                            borderRadius: 99,
                            background: active
                              ? `${accent}22`
              ...
              {/* TYPE SELECTOR */}
              <label style={{ ...S.lbl, marginTop: 4, marginBottom: 7 }}>
                Tipo de despesa
              </label>
              <TypeSelector
                value={expForm.tipo}
                onChange={(v) => setExpForm((f) => ({ ...f, tipo: v }))}
              />
              <button style={S.btnAdd} onClick={addExpense}>
                + Adicionar Despesa
              </button>
            </div>
            ...
            >
              <span style={{ fontSize: 12, color: '#64748b' }}>
                {myExpenses.length} despesa(s)
              </span>
              <span style={{ fontSize: 17, fontWeight: 800, color: '#fb7185' }}>
                {fmt(totalExp)}
              </span>
            </div>
            ...
            {myExpenses.length > 0 && (
              ...
              >
                {Object.entries(TYPE_META).map(([type, meta]) => {
                  ...
                  const target = BUDGET_TARGETS[type];
                  const over =
                    actual > totalExp * (target / 100) && totalExp > 0;
                  return (
                    <div
                      key={type}
                      style={{
                        background: over ? '#450a0a' : meta.bg,
                        border: `1px solid ${over ? '#ef4444' : meta.color}40`,
                        borderRadius: 10,
                        padding: '10px 8px',
                        textAlign: 'center',
                      }}
            ...
            <div style={S.card}>
              {myExpenses.length === 0 && (
                ...
                >
                  Sem despesas para este período. ☝️
                </div>
              )}
              {[...myExpenses].reverse().map((e) => {
        ...
        {tab === 'rendimentos' && (
          <>
              ...
              <div style={S.row2}>
                ...
                <div>
                  ...
                  <input
                    ...
                    value={incForm.valor}
                    onChange={(e) =>
                      setIncForm((f) => ({ ...f, valor: e.target.value }))
                    }
                  />
                </div>
              </div>
              <div style={S.row2}>
                <div>
                  ...
                  <select
                    ...
                    value={incForm.cat}
                    onChange={(e) =>
                      setIncForm((f) => ({ ...f, cat: e.target.value }))
                    }
                  >
                    <option value="">Selecionar...</option>
                    {incCats.map((c) => (
                ...
                <div>
                  ...
                  <input
                    ...
                    value={incForm.data}
                    onChange={(e) =>
                      setIncForm((f) => ({ ...f, data: e.target.value }))
                    }
                  />
                </div>
              </div>
              <button style={S.btnAdd} onClick={addIncome}>
            ...
            >
              <span style={{ fontSize: 12, color: '#64748b' }}>
                {myIncomes.length} entrada(s)
              </span>
              <span style={{ fontSize: 17, fontWeight: 800, color: '#34d399' }}>
                {fmt(totalInc)}
              </span>
            </div>
            ...
            <div style={S.card}>
              {myIncomes.length === 0 && (
                ...
                >
                  Sem rendimentos registados. ☝️
                </div>
              )}
              {[...myIncomes].reverse().map((i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        setIncomes((p) => p.filter((x) => x.id !== i.id))
                      }
                      style={{
                        background: 'none',
                        border: 'none',
          ...
          <>
            <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
              {['2024', '2025', '2026'].map((y) => (
                <button
                  key={y}
                  onClick={() => setRevYear(y)}
                  style={{
                    flex: 1,
                    padding: '9px 0',
                    border: `1px solid ${
                      revYear === y ? accent : 'rgba(255,255,255,0.08)'
                    }`,
                    borderRadius: 9,
                    background: revYear === y ? `${accent}22` : 'transparent',
                    color: revYear === y ? accent : '#64748b',
                    fontWeight: 700,
                    fontSize: 13,
                    cursor: 'pointer',
                    fontFamily: "'Sora',sans-serif",
            ...
            <div style={S.sg}>
              <StatCard
                label={`Receita ${revYear}`}
                value={revArr.reduce((s, v) => s + v, 0)}
                color="#34d399"
              />
              <StatCard
                label={`Despesas ${revYear}`}
                value={expenses
                  .filter(
                    (e) =>
                      e.world === world &&
            ...
            <div style={S.card}>
              <SectionTitle>
                Receita mensal — {revYear} (toca para editar)
              </SectionTitle>
              <div
                style={{
                  display: 'grid',
                  ...
                  return (
                    <div
                      ...
                      style={{
                        background: 'rgba(255,255,255,0.04)',
                        border: `1px solid ${
                          revEdit === i ? accent : 'rgba(255,255,255,0.07)'
                        }`,
                        borderRadius: 10,
                        padding: '10px 12px',
                        cursor: 'pointer',
                    ...
                    >
                      ...
                      {revEdit === i ? (
                        <input
                          ...
                          onBlur={saveRev}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') saveRev();
                            if (e.key === 'Escape') setRevEdit(null);
                          }}
                        />
                      ) : (
                        <>
