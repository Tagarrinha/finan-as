...
type NavTab = "resumo"|"despesas"|"rendimentos"|"objetivos"|"comparacao"|"casal"|"exportar"|"definicoes"|"recorrentes";
...
const [successMsg, setSuccessMsg] = useState("");
  ...
  async function addIncome(){if(!incForm.descricao.trim()||!incForm.valor||!incForm.cat)return;const{data,error}=await supabase.from("incomes").insert({user_id:user.id,...incForm,valor:Number(incForm.valor),world}).select().single();if(!error&&data){setIncomes(p=>[data as Income,...p]);setIncForm(f=>({...f,descricao:"",valor:""}));setSuccessMsg("✓ Rendimento adicionado!");setTimeout(()=>setSuccessMsg(""),3000);}}
...
<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:4}}>
  ...
  <div style={{background:T.cardBg,border:`1px solid ${T.cardBorder}`,borderRadius:16,padding:"20px",marginBottom:14,boxShadow:"0 1px 3px rgba(0,0,0,0.3),0 4px 16px rgba(0,0,0,0.2)"}}>
    {/* Header */}
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14,paddingBottom:12,borderBottom:`1px solid ${T.cardBorder}`}}>
      <div style={{width:28,height:28,borderRadius:8,background:"linear-gradient(135deg,#57E3A0,#00a86b)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>💶</div>
      <div style={{fontSize:13,fontWeight:800,color:"#f1f5f9"}}>{editingInc?"Editar rendimento":"Novo rendimento"}</div>
    </div>
    {/* Descrição + Valor */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
    ...
    {/* Botão */}
    <button onClick={()=>editingInc?updateIncome(editingInc):addIncome()} style={{width:"100%",padding:"15px 0",background:editingInc?"linear-gradient(135deg,#f59e0b,#d97706)":"linear-gradient(135deg,#57E3A0,#00a86b)",border:"none",borderRadius:12,color:"#fff",fontWeight:800,fontSize:15,cursor:"pointer",fontFamily:"'Sora',sans-serif",boxShadow:editingInc?"0 4px 24px rgba(245,158,11,0.4)":"0 4px 24px rgba(87,227,160,0.35)",letterSpacing:"-0.2px"}}>
      {editingInc?"✓ Guardar alterações":"+ Adicionar rendimento"}
    </button>
    {editingInc&&<button onClick={()=>{setEditingInc(null);setIncForm(f=>({...f,descricao:"",valor:""}));}} style={{width:"100%",marginTop:8,padding:"10px 0",background:"rgba(255,255,255,0.05)",border:`1px solid ${T.cardBorder}`,borderRadius:10,color:T.subtext,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"'Sora',sans-serif"}}>✕ Cancelar edição</button>}
  </div>
  ...
  {/* Lista */}
  <div style={S.card}>
    {myIncomes.filter(i=>!searchInc||i.descricao.toLowerCase().includes(searchInc.toLowerCase())||incCats.find(c=>c.id===i.cat)?.label.toLowerCase().includes(searchInc.toLowerCase())).length===0&&<div style={{color:T.subtext,fontSize:13,textAlign:"center",padding:"24px 0"}}>{searchInc?`Sem resultados para "${searchInc}"`:"Sem rendimentos registados."}</div>}
    {myIncomes.filter(i=>!searchInc||i.descricao.toLowerCase().includes(searchInc.toLowerCase())||incCats.find(c=>c.id===i.cat)?.label.toLowerCase().includes(searchInc.toLowerCase())).map(i=>{const cat=incCats.find(c=>c.id===i.cat);return(
      <div key={i.id} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 0",borderBottom:`1px solid ${T.cardBorder}`}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:11,color:T.subtext,marginTop:2}}>{new Date(i.data+"T12:00:00").toLocaleDateString("pt-PT")} · {cat?.label}</div>
        </div>
        <span style={{fontSize:14,fontWeight:700,color:T.positive,minWidth:68,textAlign:"right"}}>{fmt(Number(i.valor))}</span>
        <button onClick={()=>{setEditingInc(i.id);setIncForm({descricao:i.descricao,valor:String(i.valor),cat:i.cat,data:i.data});window.scrollTo({top:0,behavior:"smooth"});}} style={{background:"none",border:"none",cursor:"pointer",color:T.accent,fontSize:15,padding:"0 2px"}}>✏️</button>
        <button onClick={()=>deleteIncome(i.id)} style={{background:"none",border:"none",cursor:"pointer",color:T.subtext,fontSize:15,padding:"0 2px"}}>✕</button>
      </div>
    );})}
  </div>
      ...
      {byIncCat.filter(c=>c.total>0).map(c=>(
        <div key={c.id} style={{marginBottom:10}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
            ...
            <span style={{fontSize:13,fontWeight:700,color:T.positive}}>{fmt(c.total)} <span style={{fontSize:11,color:T.subtext,fontWeight:400}}>({pct(c.total,totalInc)}%)</span></span>
          </div>
          <ProgressBar value={c.total} max={maxInc} color={T.positive} height={4}/>
        </div>
      ))}
    </div>
  )}
