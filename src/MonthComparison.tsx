...
export default function MonthComparison({ expenses, incomes, expCats, world, accent, cardBg, cardBorder, subtext, positive, negative, lang }: Props) {
  const now = new Date();
  const currMonth = now.getMonth();
  const currYear  = now.getFullYear();
  const MONTHS_FULL  = lang==="pt" ? MONTHS_PT : MONTHS_EN;
  const MONTHS_SHORT = lang==="pt" ? MONTHS_SHORT_PT : MONTHS_SHORT_EN;
  ...
  const months = useMemo(() => {
    ...
    for(let i = 5; i >= 0; i--) {
      ...
      result.push({ month:m, year:y, label:MONTHS_SHORT[m], fullLabel:MONTHS_FULL[m], totalExp, totalInc, poupanca:totalInc-totalExp, byCat, byType, expCount:exp.length });
    }
    return result;
  }, [expenses, incomes, world, expCats, lang]);
  ...
  const insight = useMemo(() => {
    ...
    if(lang==="en") {
      if(diffPoup > 0)
        return { type:"good", text:`Great! You saved ${fmt(diffPoup)} more than in ${prev.fullLabel}. ${biggestIncrease.diff<0?`You reduced ${biggestIncrease.icon} ${biggestIncrease.label} by ${fmt(Math.abs(biggestIncrease.diff))}.`:""}` };
      if(diffExp > 0)
        return { type:"warn", text:`You spent ${fmt(Math.abs(diffExp))} more than in ${prev.fullLabel}. The category with the biggest increase was ${topCat.icon} ${topCat.label} (${fmt(curr.byCat[topCat.id]||0)}).` };
      return { type:"neutral", text:`Your expenses remained stable compared to ${prev.fullLabel}.` };
    }
    if(diffPoup > 0)
  ...
  if(months.every(m=>m.totalExp===0&&m.totalInc===0)) return (
    <div style={{ textAlign:"center", padding:"48px 0", color:subtext, fontFamily:"'Sora',sans-serif" }}>
      ...
      <div style={{ fontSize:15, fontWeight:700, color:"#e2e8f0", marginBottom:6 }}>{t[lang].compNoData}</div>
      <div style={{ fontSize:13, lineHeight:1.6 }}>{t[lang].compNoDataDesc.split("\n").map((l,i)=><span key={i}>{l}{i===0&&<br/>}</span>)}</div>
    </div>
  );
  ...
  return (
    <div style={{ fontFamily:"'Sora',sans-serif" }}>
      {insight&&(
        <div style={{ background:insight.type==="good"?"rgba(52,211,153,0.08)":insight.type==="warn"?"rgba(251,113,133,0.08)":"rgba(255,255,255,0.04)", border:`1px solid ${insight.type==="good"?"rgba(52,211,153,0.3)":insight.type==="warn"?"rgba(251,113,133,0.3)":"rgba(255,255,255,0.08)"}`, borderRadius:14, padding:"14px 16px", marginBottom:14 }}>
          <div style={{ fontSize:12, fontWeight:700, color:insight.type==="good"?positive:insight.type==="warn"?negative:"#94a3b8", marginBottom:5 }}>
            {insight.type==="good"?t[lang].compGood:insight.type==="warn"?t[lang].compWarn:t[lang].compAnalysis}
          </div>
      ...
      <div style={{ ...S.card }}>
        ...
        {[
          { label:t[lang].compIncome,   curr:curr.totalInc, prev:prev.totalInc, color:positive, inverse:false },
          { label:t[lang].compExpenses, curr:curr.totalExp, prev:prev.totalExp, color:negative, inverse:true  },
          { label:t[lang].compSavings,  curr:curr.poupanca, prev:prev.poupanca, color:accent,   inverse:false },
        ].map(row=>(
          <div key={row.label} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 0", borderBottom:`1px solid ${cardBorder}` }}>
            ...
            <span style={{ fontSize:11, color:subtext, flex:"0 0 80px", textAlign:"right" as const }}>{t[lang].compPrev} {fmt(row.prev)}</span>
          </div>
        ))}
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", paddingTop:10 }}>
          <span style={{ fontSize:12, color:subtext }}>{t[lang].compAccumulated} ({currYear})</span>
          <span style={{ fontSize:15, fontWeight:800, color:"#a78bfa" }}>{fmt(totalPoupanca)}</span>
        </div>
      </div>

      <div style={{ ...S.card }}>
        <span style={S.secTitle}>{t[lang].compEvolution}</span>
        ...
        <div style={{ display:"flex", gap:14 }}>
          <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color:"#94a3b8" }}><div style={{ width:10,height:10,borderRadius:2,background:positive }}/>{t[lang].compIncome}</div>
          <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color:"#94a3b8" }}><div style={{ width:10,height:10,borderRadius:2,background:negative }}/>{t[lang].compExpenses}</div>
        </div>
      </div>

      <div style={{ ...S.card }}>
        <span style={S.secTitle}>{t[lang].compCategories} — {curr.fullLabel} vs {prev.fullLabel}</span>
        {expCats.map(c=>{
            ...
            <div key={c.id} style={{ marginBottom:12 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:5 }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  {pVal>0&&<span style={{ fontSize:11, color:subtext }}>{t[lang].compPrev} {fmt(pVal)}</span>}
              ...
              {diff!==0&&<div style={{ fontSize:10, color:diff>0?"#fb7185":"#34d399", marginTop:3, fontWeight:600 }}>{diff>0?"↑":"↓"} {fmt(Math.abs(diff))} {t[lang].compVsPrev}</div>}
            </div>
          );
        })}
      </div>
      ...
      <div style={{ ...S.card }}>
        <span style={S.secTitle}>{t[lang].compSavingsTrend}</span>
        {months.map((m,i)=>{
