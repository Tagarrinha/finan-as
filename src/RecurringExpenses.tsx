...
export default function RecurringExpenses({ userId, world, expCats, accent, accentDark, cardBg, cardBorder, subtext, positive, negative, recurring, setRecurring, onApplyDue, lang }: Props) {
  ...
  async function deleteRecurring(id:number) {
    if(!window.confirm(t[lang].recurringDeleteConfirm)) return;
    ...
    <div>
      {dueNow.length>0&&(
        <div style={{background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.3)",borderRadius:12,padding:"12px 14px",marginBottom:14}}>
          <div style={{fontSize:12,fontWeight:700,color:"#f59e0b",marginBottom:8}}>🔔 {dueNow.length} {dueNow.length>1?t[lang].recurringDuePlural:t[lang].recurringDue}!</div>
          {dueNow.map(r=>{
            ...
            return(
              <div key={r.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:"1px solid rgba(245,158,11,0.15)"}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:11,color:"#94a3b8"}}>{freqLabels[r.frequencia].label} · {new Date(r.proxima_data+"T12:00:00").toLocaleDateString(lang==="pt"?"pt-PT":"en-GB")}</div>
                </div>
                <div style={{fontSize:14,fontWeight:800,color:negative}}>{fmt(Number(r.valor))}</div>
                <button onClick={()=>applyAndAdvance(r)} style={{padding:"6px 12px",background:`${accent}22`,border:`1px solid ${accent}50`,borderRadius:8,color:accent,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"'Sora',sans-serif",whiteSpace:"nowrap"}}>{t[lang].recurringRegister}</button>
              </div>
            );
          })}
        </div>
      ...
      <button onClick={()=>{if(showForm&&!editingId){resetForm();}else{setEditingId(null);setForm(EMPTY_FORM);setShowForm(true);}}} style={{width:"100%",marginBottom:14,padding:"11px 0",background:showForm?`${accent}18`:`linear-gradient(135deg,${accent},${accentDark})`,border:showForm?`1px solid ${accent}40`:"none",borderRadius:10,color:showForm?accent:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"'Sora',sans-serif",transition:"all .2s"}}>
        {showForm?(editingId?t[lang].cancelEdit:t[lang].cancel):t[lang].recurringNew}
      </button>
      {showForm&&(
        <div style={{background:cardBg,border:`1px solid ${editingId?"#f59e0b":accent}40`,borderRadius:14,padding:"16px 18px",marginBottom:14}}>
          <div style={{fontSize:11,fontWeight:700,color:editingId?"#f59e0b":accent,textTransform:"uppercase" as const,letterSpacing:"0.08em",marginBottom:14}}>
            {editingId?t[lang].recurringEditTitle:t[lang].recurringNewTitle}
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
            <div><label style={lbl}>{t[lang].recurringDescription}</label><input style={inp} placeholder={lang==="pt"?"Ex: Ginásio":"E.g. Gym"} value={form.descricao} onChange={e=>setForm(f=>({...f,descricao:e.target.value}))}/></div>
            <div><label style={lbl}>{t[lang].recurringValue}</label><input style={inp} type="number" placeholder="0,00" value={form.valor} onChange={e=>setForm(f=>({...f,valor:e.target.value}))}/></div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
            <div>
              <label style={lbl}>{t[lang].recurringCategory}</label>
          ...
          {selCat?.sub&&(
            <div style={{marginBottom:10}}>
              <label style={lbl}>{t[lang].subCategory}</label>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
            <div>
              <label style={lbl}>{t[lang].recurringType}</label>
          ...
          <button onClick={saveRecurring} disabled={saving} style={{width:"100%",padding:"11px 0",background:`linear-gradient(135deg,${editingId?"#f59e0b":accent},${editingId?"#d97706":accentDark})`,border:"none",borderRadius:9,color:"#fff",fontWeight:700,fontSize:13,cursor:saving?"not-allowed":"pointer",fontFamily:"'Sora',sans-serif",opacity:saving?0.7:1}}>
            {saving?t[lang].recurringSaving:(editingId?t[lang].saveChanges:t[lang].recurringCreate)}
          </button>
      ...
      {myRecurring.length===0&&!showForm&&(
        <div style={{textAlign:"center" as const,color:subtext,fontSize:13,padding:"32px 0"}}>
          ...
          <div style={{fontWeight:600,color:"#e2e8f0",marginBottom:4}}>{t[lang].recurringEmpty}</div>
          <div>{t[lang].recurringEmptyDesc}</div>
        </div>
      )}
      {myRecurring.map(r=>{
        const cat=expCats.find(c=>c.id===r.cat);
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:6,marginTop:3,flexWrap:"wrap" as const}}>
                  ...
                  {isDue&&<span style={{fontSize:10,background:"rgba(245,158,11,0.2)",color:"#f59e0b",padding:"2px 7px",borderRadius:99,fontWeight:700}}>{t[lang].recurringDueLabel}</span>}
                </div>
              </div>
              <div style={{textAlign:"right" as const}}>
                <div style={{fontSize:16,fontWeight:800,color:negative}}>{fmt(Number(r.valor))}</div>
                <div style={{fontSize:10,color:subtext,marginTop:2}}>{new Date(r.proxima_data+"T12:00:00").toLocaleDateString(lang==="pt"?"pt-PT":"en-GB")}</div>
              </div>
            </div>
            <div style={{display:"flex",gap:6}}>
              {isDue&&<button onClick={()=>applyAndAdvance(r)} style={{flex:2,padding:"7px 0",background:`${accent}22`,border:`1px solid ${accent}40`,borderRadius:8,color:accent,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"'Sora',sans-serif"}}>{t[lang].recurringRegisterNow}</button>}
              <button onClick={()=>openEdit(r)} style={{flex:1,padding:"7px 0",background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:8,color:"#f59e0b",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"'Sora',sans-serif"}}>{t[lang].coupleEdit}</button>
      ...
      {myRecurring.filter(r=>r.ativa).length>0&&(
        <div style={{background:cardBg,border:`1px solid ${cardBorder}`,borderRadius:12,padding:"12px 16px",marginTop:8}}>
          <div style={{fontSize:10,fontWeight:700,color:subtext,textTransform:"uppercase" as const,letterSpacing:"0.08em",marginBottom:8}}>{t[lang].recurringSummary}</div>
