...
export default function SavingsGoals({ userId, accent, accentDark, cardBg, cardBorder, subtext, positive, negative, goals, setGoals, monthlyIncome, maxGoals, onUpgrade }: Props) {
  ...
  function openEditCouple(g: CoupleGoal) {
    setEditingCoupleId(g.id);
    setCoupleForm({ nome:g.nome, icon:g.icon, objetivo:String(g.objetivo), prazo:g.prazo });
    setShowCoupleForm(true);
    window.scrollTo({ top:0, behavior:"smooth" });
  }
  function resetCoupleForm() { setCoupleForm(EMPTY_COUPLE_FORM); setEditingCoupleId(null); setShowCoupleForm(false); }
  async function saveCoupleGoal() {
    ...
    if(editingCoupleId) {
      await supabase.from("couple_goals").update({ nome:coupleForm.nome.trim(), icon:coupleForm.icon, objetivo:Number(coupleForm.objetivo), prazo:coupleForm.prazo }).eq("id",editingCoupleId);
      setCoupleGoals(p=>p.map(g=>g.id===editingCoupleId?{...g,nome:coupleForm.nome.trim(),icon:coupleForm.icon,objetivo:Number(coupleForm.objetivo),prazo:coupleForm.prazo}:g));
    } else {
    ...
    resetCoupleForm(); setSavingCouple(false);
  }
    ...
    <div>
      ...
      {mode==="pessoal"&&<>
        {goals.length > 0 && (
          <div style={{background:`linear-gradient(135deg,${accent}18,${accentDark}10)`,border:`1px solid ${accent}30`,borderRadius:14,padding:"12px 16px",marginBottom:12,display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
            <div style={{textAlign:"center" as const}}><div style={{fontSize:9,color:subtext,textTransform:"uppercase" as const,letterSpacing:"0.07em",marginBottom:3}}>Poupado</div><div style={{fontSize:15,fontWeight:800,color:positive}}>{fmt(totalAtual)}</div></div>
            <div style={{textAlign:"center" as const,borderLeft:`1px solid ${cardBorder}`,borderRight:`1px solid ${cardBorder}`}}><div style={{fontSize:9,color:subtext,textTransform:"uppercase" as const,letterSpacing:"0.07em",marginBottom:3}}>Meta total</div><div style={{fontSize:15,fontWeight:800,color:accent}}>{fmt(totalMeta)}</div></div>
            <div style={{textAlign:"center" as const}}><div style={{fontSize:9,color:subtext,textTransform:"uppercase" as const,letterSpacing:"0.07em",marginBottom:3}}>Atingidos</div><div style={{fontSize:15,fontWeight:800,color:"#a78bfa"}}>{done}/{goals.filter(g=>g.ativa).length}</div></div>
          </div>
        )}
        <button onClick={()=>{
        ...
        {showForm&&(
          <div style={{background:cardBg,border:`1px solid ${editingId?"#f59e0b":accent}40`,borderRadius:14,padding:"16px 18px",marginBottom:12}}>
            <div style={{fontSize:11,fontWeight:700,color:editingId?"#f59e0b":accent,textTransform:"uppercase" as const,marginBottom:14}}>{editingId?"✏️ Editar objetivo":"Novo objetivo"}</div>
      ...
      {mode==="partilhada"&&<>
        ...
        {!loadingCouple&&couple&&<>
          ...
          <button onClick={()=>{if(showCoupleForm&&!editingCoupleId){resetCoupleForm();}else{resetCoupleForm();setShowCoupleForm(true);}}} style={{width:"100%",marginBottom:12,padding:"11px 0",background:showCoupleForm&&!editingCoupleId?"rgba(236,72,153,0.12)":"linear-gradient(135deg,#ec4899,#f97316)",border:showCoupleForm&&!editingCoupleId?"1px solid rgba(236,72,153,0.3)":"none",borderRadius:10,color:showCoupleForm&&!editingCoupleId?"#ec4899":"#fff",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"'Sora',sans-serif"}}>
            {showCoupleForm&&!editingCoupleId?"✕ Cancelar":"+ Nova meta partilhada"}
          </button>
          {showCoupleForm&&(
            <div style={{background:cardBg,border:`1px solid ${editingCoupleId?"#f59e0b":"rgba(236,72,153,0.3)"}`,borderRadius:14,padding:"16px 18px",marginBottom:12}}>
              <div style={{fontSize:11,fontWeight:700,color:editingCoupleId?"#f59e0b":"#ec4899",textTransform:"uppercase" as const,marginBottom:14}}>{editingCoupleId?"✏️ Editar meta partilhada":"Nova meta partilhada 💑"}</div>
              <div style={{display:"flex",gap:8,marginBottom:10}}>
              ...
              <button onClick={saveCoupleGoal} disabled={savingCouple} style={{width:"100%",padding:"11px 0",background:editingCoupleId?"linear-gradient(135deg,#f59e0b,#d97706)":"linear-gradient(135deg,#ec4899,#f97316)",border:"none",borderRadius:9,color:"#fff",fontWeight:700,fontSize:13,cursor:savingCouple?"not-allowed":"pointer",fontFamily:"'Sora',sans-serif",opacity:savingCouple?0.7:1}}>
                {savingCouple?"A guardar...":(editingCoupleId?"✓ Guardar alterações":"Criar meta partilhada →")}
              </button>
              {editingCoupleId&&<button onClick={resetCoupleForm} style={{width:"100%",marginTop:8,padding:"9px 0",background:"rgba(255,255,255,0.04)",border:`1px solid ${cardBorder}`,borderRadius:9,color:subtext,fontSize:13,cursor:"pointer",fontFamily:"'Sora',sans-serif"}}>✕ Cancelar</button>}
            </div>
          )}
                ...
                <div key={g.id}>
                  <div style={{display:"flex",alignItems:"center",gap:10,padding:"12px 0",borderBottom:`1px solid ${cardBorder}`}}>
                    ...
                    <div style={{display:"flex",gap:4,flexShrink:0}}>
                      ...
                      <button onClick={()=>openEditCouple(g)} style={{width:28,height:28,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:7,color:subtext,fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>✏️</button>
