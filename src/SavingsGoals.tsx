...
interface CoupleGoal {
  ...
  prazo: string;
  icon: string;
  contribuicao_user1: number;
  contribuicao_user2: number;
  created_by: string;
}
...
export default function SavingsGoals({ userId, accent, accentDark, cardBg, cardBorder, subtext, positive, negative, goals, setGoals, monthlyIncome, maxGoals, onUpgrade }: Props) {
  ...
  useEffect(() => {
    if(mode === "partilhada") loadCoupleData();
  }, [mode]);
  ...
  async function saveCoupleGoal() {
    ...
    const row = {
      couple_id: couple.id,
      nome: coupleForm.nome.trim(),
      icon: coupleForm.icon,
      objetivo: Number(coupleForm.objetivo),
      prazo: coupleForm.prazo,
      contribuicao_user1: 0,
      contribuicao_user2: 0,
      created_by: userId,
    };
    const { data, error } = await supabase.from("couple_goals").insert(row).select().single();
    if(!error && data) setCoupleGoals(p=>[...p, data as CoupleGoal]);
    setCoupleForm(EMPTY_COUPLE_FORM);
  ...
  async function addCoupleDeposit(goal: CoupleGoal) {
    ...
    await supabase.from("couple_goals").update({ [field]: newVal }).eq("id", goal.id);
    setCoupleGoals(p=>p.map(g=>g.id===goal.id?{...g,[field]:newVal}:g));
    setShowCoupleDeposit(null);
    setCoupleDepositVal("");
  }
  ...
  const myName = userId ? (isUser1 ? couple?.user1_email?.split("@")[0] : couple?.user2_email?.split("@")[0]) : "Tu";
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",background:"rgba(255,255,255,0.04)",borderRadius:12,padding:4,marginBottom:16,gap:4}}>
        <button
          onClick={()=>{setMode("pessoal");setShowForm(false);}}
          style={{flex:1,padding:"10px 0",border:"none",borderRadius:9,background:mode==="pessoal"?`linear-gradient(135deg,${accent},${accentDark})`:"transparent",color:mode==="pessoal"?"#fff":subtext,fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"'Sora',sans-serif",transition:"all .2s"}}
        >
          👤 Pessoais
        </button>
        <button
          onClick={()=>{setMode("partilhada");setShowForm(false);}}
          style={{flex:1,padding:"10px 0",border:"none",borderRadius:9,background:mode==="partilhada"?"linear-gradient(135deg,#ec4899,#f97316)":"transparent",color:mode==="partilhada"?"#fff":subtext,fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"'Sora',sans-serif",transition:"all .2s"}}
        >
          💑 Partilhadas
        </button>
      </div>
            ...
            <div key={g.id} style={{background:isDone?`${g.cor}0d`:cardBg,border:`1px solid ${isDone?g.cor:g.ativa?cardBorder:"rgba(255,255,255,0.03)"}`,borderRadius:16,padding:"16px 18px",marginBottom:12,opacity:g.ativa?1:0.5,transition:"all .2s"}}>
              ...
              <div style={{marginBottom:10}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
                  <span style={{fontSize:11,color:subtext}}>{pct}% {monthlyIncome > 0 ? "do rendimento mensal" : "concluído"}</span>
      ...
      ══════════════════════════════════════════════════════ */}
      {mode==="partilhada"&&<>
        {loadingCouple&&<div style={{textAlign:"center",padding:"32px 0",color:subtext}}>A carregar...</div>}
        ...
        {!loadingCouple&&couple&&<>
          ...
          {coupleGoals.length>0&&(()=>{
            ...
            const done = coupleGoals.filter(g=>(Number(g.contribuicao_user1)+Number(g.contribuicao_user2))>=Number(g.objetivo)).length;
            return(
              <div style={{background:"linear-gradient(135deg,rgba(236,72,153,0.12),rgba(249,115,22,0.08))",border:"1px solid rgba(236,72,153,0.25)",borderRadius:16,padding:"16px 18px",marginBottom:16,display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
                <div style={{textAlign:"center"}}>
                  <div style={{fontSize:10,color:subtext,textTransform:"uppercase" as const,letterSpacing:"0.07em",marginBottom:4}}>Poupado</div>
                  <div style={{fontSize:17,fontWeight:800,color:positive}}>{fmt(totalPoupado)}</div>
                </div>
                <div style={{textAlign:"center",borderLeft:`1px solid ${cardBorder}`,borderRight:`1px solid ${cardBorder}`}}>
                  <div style={{fontSize:10,color:subtext,textTransform:"uppercase" as const,letterSpacing:"0.07em",marginBottom:4}}>Objetivo</div>
                  <div style={{fontSize:17,fontWeight:800,color:"#ec4899"}}>{fmt(totalObj)}</div>
                </div>
                <div style={{textAlign:"center"}}>
                  <div style={{fontSize:10,color:subtext,textTransform:"uppercase" as const,letterSpacing:"0.07em",marginBottom:4}}>Atingidos</div>
                  <div style={{fontSize:17,fontWeight:800,color:"#a78bfa"}}>{done}/{coupleGoals.length}</div>
                </div>
              </div>
            );
          })()}
          ...
          {/* Botão adicionar */}
          <button onClick={()=>setShowCoupleForm(v=>!v)} style={{width:"100%",marginBottom:14,padding:"11px 0",background:showCoupleForm?"rgba(236,72,153,0.12)":"linear-gradient(135deg,#ec4899,#f97316)",border:showCoupleForm?"1px solid rgba(236,72,153,0.3)":"none",borderRadius:10,color:showCoupleForm?"#ec4899":"#fff",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"'Sora',sans-serif"}}>
            {showCoupleForm?"✕ Cancelar":"+ Nova meta partilhada"}
          </button>

          {/* Formulário */}
          {showCoupleForm&&(
            <div style={{background:cardBg,border:"1px solid rgba(236,72,153,0.3)",borderRadius:14,padding:"16px 18px",marginBottom:14}}>
              <div style={{fontSize:11,fontWeight:700,color:"#ec4899",textTransform:"uppercase" as const,letterSpacing:"0.08em",marginBottom:14}}>Nova meta partilhada 💑</div>
              <div style={{display:"flex",gap:8,marginBottom:10}}>
                <input style={{...inp,flex:"0 0 56px",textAlign:"center",fontSize:22}} placeholder="🎯" value={coupleForm.icon} onChange={e=>setCoupleForm(f=>({...f,icon:e.target.value}))} maxLength={2}/>
                <input style={{...inp,flex:1}} placeholder="Nome da meta (ex: Férias, Casa...)" value={coupleForm.nome} onChange={e=>setCoupleForm(f=>({...f,nome:e.target.value}))}/>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
                <div>
                  <div style={{fontSize:10,fontWeight:700,color:subtext,textTransform:"uppercase" as const,letterSpacing:"0.07em",marginBottom:5}}>Objetivo (€)</div>
                  <input style={inp} type="number" placeholder="0,00" value={coupleForm.objetivo} onChange={e=>setCoupleForm(f=>({...f,objetivo:e.target.value}))}/>
                </div>
                <div>
                  <div style={{fontSize:10,fontWeight:700,color:subtext,textTransform:"uppercase" as const,letterSpacing:"0.07em",marginBottom:5}}>Prazo</div>
                  <input style={inp} type="date" value={coupleForm.prazo} onChange={e=>setCoupleForm(f=>({...f,prazo:e.target.value}))}/>
                </div>
              </div>
          ...
          {coupleGoals.length===0&&!showCoupleForm&&(
            <div style={{textAlign:"center" as const,padding:"40px 0",color:subtext}}>
              ...
              <div style={{fontSize:13,lineHeight:1.6}}>Cria a vossa primeira meta em conjunto —<br/>férias, casa, carro, fundo de emergência...</div>
            </div>
          )}

          {/* Lista metas partilhadas */}
          {coupleGoals.map(g=>{
            ...
            const myPct = total > 0 ? Math.round((myContrib/total)*100) : 50;

            return(
              <div key={g.id} style={{background:isDone?"rgba(52,211,153,0.06)":cardBg,border:`1px solid ${isDone?"rgba(52,211,153,0.3)":cardBorder}`,borderRadius:16,padding:"16px 18px",marginBottom:12}}>
                {/* Header */}
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <div style={{width:42,height:42,borderRadius:12,background:"rgba(236,72,153,0.15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>{g.icon}</div>
                    <div>
                      <div style={{fontSize:14,fontWeight:700,color:"#f1f5f9"}}>{g.nome}</div>
                      <div style={{fontSize:11,color:subtext,marginTop:2}}>Prazo: {prazo.toLocaleDateString("pt-PT",{month:"long",year:"numeric"})}</div>
                    </div>
                  </div>
                  {isDone&&<div style={{background:"rgba(52,211,153,0.15)",border:"1px solid rgba(52,211,153,0.3)",borderRadius:99,padding:"3px 10px",fontSize:11,fontWeight:700,color:"#34d399",flexShrink:0}}>✓ Atingido!</div>}
                </div>

                {/* Progress */}
                <div style={{marginBottom:12}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                    <span style={{fontSize:14,fontWeight:800,color:"#ec4899"}}>{fmt(total)}</span>
                    <span style={{fontSize:13,color:subtext}}>{fmt(objetivo)}</span>
                  </div>
                  <div style={{height:10,borderRadius:99,background:"rgba(255,255,255,0.07)",overflow:"hidden",marginBottom:6}}>
                    <div style={{width:`${pct}%`,height:"100%",background:isDone?"linear-gradient(90deg,#34d399,#059669)":"linear-gradient(90deg,#ec4899,#f97316)",borderRadius:99,transition:"width .6s ease"}}/>
                  </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
                  <div style={{background:`${MY_COLOR}10`,border:`1px solid ${MY_COLOR}20`,borderRadius:10,padding:"10px 12px"}}>
                    <div style={{fontSize:10,color:MY_COLOR,fontWeight:700,marginBottom:4}}>👤 {myName || "Tu"}</div>
                    <div style={{fontSize:16,fontWeight:800,color:MY_COLOR}}>{fmt(myContrib)}</div>
                    <div style={{fontSize:10,color:subtext,marginTop:2}}>{myPct}% do total</div>
                  </div>
                  <div style={{background:`${PARTNER_COLOR}10`,border:`1px solid ${PARTNER_COLOR}20`,borderRadius:10,padding:"10px 12px"}}>
                    <div style={{fontSize:10,color:PARTNER_COLOR,fontWeight:700,marginBottom:4}}>👤 {partnerName}</div>
                    <div style={{fontSize:16,fontWeight:800,color:PARTNER_COLOR}}>{fmt(partnerContrib)}</div>
                    <div style={{fontSize:10,color:subtext,marginTop:2}}>{100-myPct}% do total</div>
                  </div>
                </div>
                ...
                {!isDone&&(
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
                    ...
                    <div style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"8px 12px"}}>
                      <div style={{fontSize:10,color:subtext,marginBottom:2}}>Por mês (total)</div>
                      <div style={{fontSize:14,fontWeight:700,color:"#ec4899"}}>{meses>0?fmt(falta/meses):"—"}</div>
                    </div>
                  </div>
                )}
                ...
                <div style={{display:"flex",gap:6}}>
                  {!isDone&&(
                    <button onClick={()=>setShowCoupleDeposit(showCoupleDeposit===g.id?null:g.id)} style={{flex:2,padding:"7px 0",background:"rgba(236,72,153,0.12)",border:"1px solid rgba(236,72,153,0.25)",borderRadius:8,color:"#ec4899",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"'Sora',sans-serif"}}>
                      {showCoupleDeposit===g.id?"✕ Cancelar":"💰 Contribuir"}
                    </button>
                  )}
                  <button onClick={()=>deleteCoupleGoal(g.id)} style={{flex:1,padding:"7px 0",background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.2)",borderRadius:8,color:"#f87171",fontSize:11,cursor:"pointer",fontFamily:"'Sora',sans-serif"}}>🗑️ Apagar</button>
                </div>
              </div>
            );
          })}
